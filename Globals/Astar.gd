...
func half_size(grid):
	var _half_cell_size = grid.cell_size / 2
	return _half_cell_size

# Loops through all cells within the map's bounds and
# adds all points to the astar_node, except the obstacles
func astar_add_walkable_cells(map_size,obstacles = []):
	var points_array = []
	for y in range(map_size.y):
		for x in range(map_size.x):
			...
			astar_node.add_point(point_index, Vector3(point.x, 0.0, point.y))
	return points_array
...
func is_outside_map_bounds(map_size,point):
	return point.x < 0 or point.y < 0 or point.x >= map_size.x or point.y >= map_size.y


func calculate_point_index(map_size,point):
	return point.x + map_size.x * point.y


func get_path(grid,world_start, world_end):
	self.path_start_position = grid.world_to_map(world_start)
	self.path_end_position = grid.world_to_map(world_end)
	_recalculate_path()
	var path_world = []
	for point in _point_path:
		var point_world = grid.map_to_world(Vector2(point.x, point.y,point.z)) + half_size(grid)
...
func _set_path_start_position(value,obstacles=0):
	if value in obstacles:
		return
	if is_outside_map_bounds(value):
		return

	set_cell_item(path_start_position.x, path_start_position.y,path_start_position.z, -1)
	set_cell_item(value.x, value.y,value.z, 1)
	path_start_position = value
	if path_end_position and path_end_position != path_start_position:
		_recalculate_path()


func _set_path_end_position(value,obstacles=0):
	if value in obstacles:
		return
	if is_outside_map_bounds(value):
		return
	...
	set_cell_item(path_start_position.x, path_start_position.y,path_start_position.z, -1)
	set_cell_item(value.x, value.y,value.z, 2)
